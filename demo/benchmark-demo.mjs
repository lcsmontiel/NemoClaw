...
/**
 * Benchmark demo — visual version for terminal recording.
 * Runs locally against a temp dir (no container needed for the demo).
 * Uses the same logic as the container benchmark but with pretty output.
 */
...
async function main() {
  ...
  console.log(`${DIM}  Flat MEMORY.md vs Typed Index — tiktoken cl100k_base${RESET}`);
  console.log("");
  await sleep(1000);

  // Context window table
  console.log(`${BOLD}  Context Window Tokens (what the agent loads at session start)${RESET}`);
  console.log("");
  ...
  if (cross) {
    console.log(`  ${BOLD}Crossover:${RESET} Typed index wins at ${YELLOW}~${cross.n} entries${RESET} and above.`);
  }
  console.log("");
  await sleep(1000);

  // Session cost table
  console.log(`${BOLD}  Total Session Cost (including tool call overhead)${RESET}`);
  console.log(`  ${DIM}Agent reads K topics per session. Overhead: ~${TOOL_OVERHEAD} tokens/call.${RESET}`);
  console.log("");
  ...
  console.log(`  ${GREEN}✓${RESET} Crossover at ~${cross ? cross.n : "?"} entries — below that, flat is smaller`);
